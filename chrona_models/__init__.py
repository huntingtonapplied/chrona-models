"""
chrona-models — runnable inference package (compiled distribution, not source).
...
class Model:
    ...
    def output_channels(self) -> List[str]:
        """Physical output channel names (field crafts, e.g. n,p,psi,Jx,Jy,Jz); falls back to the
        ONNX output names for direct-input crafts."""
        chans = (self.config.get("output") or {}).get("channels")
        return list(chans) if chans else list(self._output_names)

    def predict(
        self,
        arg: Optional[Mapping[str, np.ndarray]] = None,
        *,
        inputs: Optional[Mapping[str, np.ndarray]] = None,
        field_grids: Optional[Mapping[str, np.ndarray]] = None,
    ) -> Dict[str, np.ndarray]:
        """Run the surrogate. A single positional arg is routed by craft type: **field crafts**
        (compiled featurizer present) → `predict(field_grids)`; **direct-input crafts** →
        `predict(inputs)`. The keyword forms `predict(field_grids=…)` / `predict(inputs=…)` are
        equivalent. Returns a dict of output name -> physical array."""
        if arg is not None:
            is_field = _featurizer is not None and self._contract is not None
            if is_field and field_grids is None:
                field_grids = arg
            elif not is_field and inputs is None:
                inputs = arg
        if field_grids is not None:
            return self._predict_field_grids(field_grids)
        if inputs is None:
            raise ValueError("pass a field-grid mapping (field crafts) or inputs= (a mapping of model input arrays)")
